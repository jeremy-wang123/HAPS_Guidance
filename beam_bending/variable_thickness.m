...
%% Save settling results
save('settling_results.mat', ...
    'elastic_settling', ...
    'viscous_settling', ...
    'maxwell_settling', ...
    'elastic_x_settle', ...
...
%% Comparison between rheology uniform
w_uniform = w0 .* ...
    (1 - exp(-lambda*x) .* ...
    (cos(lambda*x) + sin(lambda*x)));
viscous_uniform = viscous_beam_profile(L, x, uniform_parameters);
...
xline(ve_settling, ':', ...
    sprintf('Burgers = %.1f km', ve_settling), ...
    'LabelVerticalAlignment', 'middle', ...
    'LabelHorizontalAlignment', 'left', ...
    'HandleVisibility', 'off');
...
exportgraphics(gcf, fullfile(figure_dir,'uniform_comparison.jpg'), ...
'Resolution',300);

%% Comparisons between rheology with varying thickness
elastic_norm = elastic_beam_profile(L, x, normalized_parameters);
viscous_norm = viscous_beam_profile(L, x, normalized_parameters);
maxwell_norm = maxwell_beam_profile(L, x, normalized_parameters);
